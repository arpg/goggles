...
TEST(goggleTests, GlobalImuVelocityCostFunction)
{
  ...
  imuParameters.a_max_ = 1000.0;
  imuParameters.g_max_ = 1000.0;
  ...
  b_a = b_a_1;
	...
	*/
	...
	problem.AddParameterBlock(v_w_0.data(),3);
	...
	problem.AddParameterBlock(v_w.data(),3);
  ...
  GlobalImuVelocityCostFunction* imu_cost_func = 
			new GlobalImuVelocityCostFunction(t0, t1,
																	imuMeasurements,
																	imuParameters);
	...
	Eigen::Vector3d v0_meas = v_w_0 + 1.0e-2*Eigen::Vector3d::Random();
	Eigen::Vector3d v1_meas = v_w_1 + 1.0e-2*Eigen::Vector3d::Random();
  ...
  jacobians_minimal[7] = J7_min.data();
	...
	for (size_t i = 0; i < 3; i++)
	{
		...
		dp_1.setZero();
		dp_1[i] = dx;
		Eigen::Vector3d v_w_0_temp = v_w_0; // save initial state
		v_w_0 = v_w_0 + dp_1;
		imu_cost_func->Evaluate(parameters,residuals_p.data(),NULL);
		v_w_0 = v_w_0_temp; // reset
		v_w_0 = v_w_0 - dp_1;
		imu_cost_func->Evaluate(parameters,residuals_m.data(),NULL);
		v_w_0 = v_w_0_temp;
